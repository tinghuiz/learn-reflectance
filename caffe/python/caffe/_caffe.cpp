...
shared_ptr<Solver<Dtype> > GetSolverFromFile(const string& filename) {
  SolverParameter param;
  ReadProtoFromTextFileOrDie(filename, &param);
  return shared_ptr<Solver<Dtype> >(GetSolver<Dtype>(param));
}

shared_ptr<Solver<Dtype> > GetSolverFromString(const string& proto_txt) {
