...
void RepackLayer< Dtype >::Reshape(const vector< Blob< Dtype >* >& bottom,
  ...
  if ( operation_ == RepackParameter_Operation_PACK_IMAGE ) {
    CHECK_EQ(bottom.size(), 1) << "Packing only supports one bottom";
    ...
    if (bottom.size() == 2) {
      CHECK_GE(nW  , bottom[1]->width()) <<
        "bottom[1] width should within 1 of computed width";
      CHECK_LE(nW, bottom[1]->width()+1) <<
        "bottom[1] width should within 1 of computed width";
      CHECK_GE(nH  , bottom[1]->height()) <<
        "bottom[1] height should within 1 of computed height";
      CHECK_LE(nH, bottom[1]->height()+1) <<
        "bottom[1] height should within 1 of computed height";
        nW = bottom[1]->width();
        nH = bottom[1]->height();
    }
