...
void hdf5_load_nd_dataset_helper(
  ...
  ArrayShape blob_dims(dims.size());
  std::copy(dims.begin(), dims.end(), blob_dims.begin());
  blob->reshape(blob_dims);
}

template <>
void hdf5_load_nd_dataset<float>(hid_t file_id, const char* dataset_name_,
        int min_dim, int max_dim, Array<float>* blob) {
...
void hdf5_load_nd_dataset<double>(hid_t file_id, const char* dataset_name_,
        int min_dim, int max_dim, Array<double>* blob) {
  hdf5_load_nd_dataset_helper(file_id, dataset_name_, min_dim, max_dim, blob);
...
void hdf5_save_nd_dataset<float>(
    const hid_t file_id, const string& dataset_name, const Array<float>& blob) {
  int num_axes = blob.shape().size();
...
void hdf5_save_nd_dataset<double>(
    hid_t file_id, const string& dataset_name, const Array<double>& blob) {
  int num_axes = blob.shape().size();
  hsize_t *dims = new hsize_t[num_axes];
  for (int i = 0; i < num_axes; ++i) {
    dims[i] = blob.shape()[i];
